  ...
  return (
    <section className="buyer-card p-4">
      <div className="flex items-baseline gap-2">
        ...
        <span className="text-sm font-semibold text-asli-green">
          {Math.round(100 - (listing.price / (listing.price * 1.4)) * 100)}% off
        </span>
      </div>
      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-500">
        <span className="inline-flex items-center gap-1">
          <Truck className="h-3.5 w-3.5" aria-hidden /> Free delivery
        </span>
        <span className="inline-flex items-center gap-1">
          <Wallet className="h-3.5 w-3.5" aria-hidden /> Cash on delivery
        </span>
      </div>
      <Link
